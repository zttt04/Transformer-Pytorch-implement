...
# 定义Train文件夹路径
train_folder = os.path.join('DataProcessing', 'Val')
...
# 保存合并后的数据
combined_train_path = os.path.join(train_folder, 'val.parquet')
...
# 保存 en 数据
en_path = os.path.join(train_folder, 'en_data_val.parquet')
extracted_df[['en']].dropna().to_parquet(en_path, index=False)

# 保存 de 数据
de_path = os.path.join(train_folder, 'de_data_val.parquet')
